...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="content:encoded">
        ...
        <!-- xsl:variable name="component" select="saxon:parse-html(.)"/>
        <element>
        ...
        <!-- xsl:for-each-group select="tokenize(., '\n')[normalize-space()]"
                            group-adjacent="string(analyze-string(., '^&lt;(p|h1|h2|h3|figure)\&gt;')//*:match/*:group)">
            <xsl:choose>
                <xsl:when test="$component='h1'">
                    <xsl:apply-templates select="h1"/>
                </xsl:when>
                <xsl:when test="$component='h2'">
                    <xsl:apply-templates select="h2"/>
                </xsl:when>
                <xsl:when test="$component='h3'">
                    <xsl:apply-templates select="h3"/>
                </xsl:when>
                <xsl:when test="$component='figure'">
